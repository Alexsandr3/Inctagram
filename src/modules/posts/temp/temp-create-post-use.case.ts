 ...
 */
export class TempCreatePostCommand {
  constructor(
...
@CommandHandler(TempCreatePostCommand)
export class TempCreatePostUseCase
  extends BaseNotificationUseCase<TempCreatePostCommand, number>
  implements ICommandHandler<TempCreatePostCommand>
{
  constructor(
   ...
   * @param command
   */
  async executeUseCase(command: TempCreatePostCommand): Promise<number> {
    ...
    try {
      ...
      //save post in db
      return await this.postsRepository.savePostWithImages(post);
    } catch (e) {
      console.log(e);
