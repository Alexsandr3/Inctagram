  ...
  implements ICommandHandler<DeleteImagePostCommand>
{
  ...
  async executeUseCase(command: DeleteImagePostCommand): Promise<void> {
    ...
    const post = await this.postsRepository.findPostByOwnerIdAndUploadIds(userId, metadata);
    if (!post)
    ...
    const correctPost = await post.deleteImage(uploadId);
    //save post
    await this.postsRepository.savePost(correctPost);
  }
}
