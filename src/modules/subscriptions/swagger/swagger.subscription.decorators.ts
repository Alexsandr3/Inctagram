...
export function SwaggerDecoratorsByGetCurrentSubscription(): MethodDecorator {
  return applyDecorators(
    ...
    ApiResponse({
      ...
      type: CurrentActiveSubscriptionsViewModel,
    }),
    ApiResponse({ status: HTTP_Status.UNAUTHORIZED_401, description: 'Unauthorized' }),
 ...
 * @constructor
 */
export function SwaggerDecoratorsByCancelAutoRenewal(): MethodDecorator {
  return applyDecorators(
    ApiOperation({ summary: 'Cancel auto renewal' }),
    ApiResponse({ status: HTTP_Status.NO_CONTENT_204, description: 'Cancel auto renewal' }),
    ApiResponse({ status: HTTP_Status.UNAUTHORIZED_401, description: 'Unauthorized' }),
    ApiResponse({ status: HTTP_Status.NOT_FOUND_404, description: 'Not found' }),
  );
}
